...
> Note: Some older examples refer to Socket.IO. Archon Beta now uses HTTP polling with ETag caching for live updates. Prefer testing polling flows (200 vs 304 responses) and cache invalidation rather than socket events.
...
6. **Polling Safety**: Prefer testing ETag-aware polling functions over sockets
...
## 🛠️ Polling Testing Best Practices

### Example: ETag-aware polling + optimistic mutation
...
it('polls with ETag and returns cached on 304', async () => {
  vi.mocked(callAPIWithETag).mockResolvedValueOnce({ status: 'ok' });
  const { result } = renderHook(
    () => usePolling({ key: ['status'], fetcher: () => callAPIWithETag('/api/status'), baseInterval: 1000 }),
    { wrapper }
  );
  // assert result.current.data after first fetch
});
```

### Example: Optimistic update + rollback

```ts
import { useDatabaseMutation } from 'src/features/ui/hooks/useDatabaseMutation';
...
See the Polling & Caching guide for more examples.
  ...
  test:
    ...
    - name: Setup Node.js
      ...
      with:
        node-version: '20'
        cache: 'npm'
        cache-dependency-path: nexarch-ui-main/package-lock.json
    
    - name: Install dependencies
      working-directory: ./nexarch-ui-main
      run: npm ci
    ...
    - name: Upload coverage reports
      ...
      with:
        file: ./nexarch-ui-main/coverage/coverage-final.json
        flags: frontend
