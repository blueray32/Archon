...
```bash
# Build and start core services (Server, MCP, Frontend, Docs)
docker compose up --build -d

# Include the optional Agents service
...
When you run `docker compose up -d`, you get core services by default:

### Microservices Architecture
- **Frontend** (Port 3737): React dashboard for managing knowledge and tasks
- **API Service** (Port 8181): FastAPI server with HTTP polling for live updates
...
- **Agents Service** (Port 8052): AI processing service for document analysis
  - Note: start with `--profile agents`
...
- **Live Updates**: HTTP polling with ETag-based caching
...
graph TB
    subgraph "🚀 Docker Microservices Stack"
        ...
        API["⚡ Server Container<br/>FastAPI + HTTP Polling<br/>Archon-Server<br/>Port 8181"]
...
- Monitor crawl progress via `/api/progress/{id}` polling
