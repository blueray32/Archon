...
FastAPI Endpoints (projects_api.py)
    ↓
> Legacy Notice: Socket.IO handler details are retained for history. Archon Beta uses HTTP polling.
...
- **Polling-first updates**: Live updates use HTTP polling with ETag caching
- **Progress Tracking**: Centralized in `ProgressService` with `/api/progress/{id}` polling
