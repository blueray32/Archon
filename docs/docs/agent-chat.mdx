...
> Note: This document includes legacy Socket.IO streaming references. Current Beta uses HTTP endpoints for chat operations, with optional polling where needed.

## Overview

The Agent Chat Panel provides a live interface for interacting with AI agents. It primarily uses HTTP endpoints and polling; previous versions referenced Socket.IO for streaming.
