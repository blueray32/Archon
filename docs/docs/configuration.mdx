...
Archon uses a microservices architecture with three separate services. All services run in Docker containers and communicate via the `archon_app-network` bridge network using service names, not localhost. Live updates use efficient HTTP polling with ETag caching.
</Admonition>

```mermaid
graph TB
    subgraph "Docker Network: archon_app-network"
        subgraph "Archon-Server"
            API[FastAPI<br/>:8080]
            POLL[HTTP Polling<br/>:8181]
        end
...
| Server | Archon-Server | 8181 | 8181 | FastAPI + HTTP polling |
...
<Admonition type="info" title="Optional Assistant UI">
ChatKit is an experimental web component from OpenAI. Nexarch exposes it behind a feature flag so you can trial it without impacting the legacy chat stack.
</Admonition>

When enabled, the `/assistant` route renders the `<openai-chatkit>` custom element and proxies session creation through the backend.

**Environment variables**

```bash
# Frontend
VITE_ENABLE_CHATKIT=1                     # toggles the Assistant route
VITE_CHATKIT_WORKFLOW_ID=wf_123...        # workflow configured in OpenAI ChatKit

# Backend
CHATKIT_WORKFLOW_ID=wf_123...             # matches the frontend workflow
OPENAI_API_BASE=https://api.openai.com    # override for regional deployments (optional)
CHATKIT_SESSION_PATH=/v1/chatkits/sessions
```

**Domain allowlist**

- Add your Nexarch origin (dev and staging) to the ChatKit domain allowlist in the OpenAI dashboard
- Staging deployments usually require both `https://app.your-domain` and the static asset host if different

**Error handling**

- Missing env vars raise a 500 with a descriptive error
- Network failures are proxied as `502` with the upstream response body for fast debugging
...
# Include optional Agents service
...
- **Location**: `nexarch-ui-main/src/services/serverHealthService.ts`
