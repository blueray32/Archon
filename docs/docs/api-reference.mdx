...
Use HTTP endpoints with ETag caching to retrieve knowledge items efficiently.

### Progress Tracking

Poll `/api/progress/{operation_id}` for crawl progress. The endpoint returns `304 Not Modified` when unchanged and `ETag` headers for caching.
...
- **🔄 Live Updates**: HTTP polling for each processing step
...
Use `/api/mcp/status` and `/api/mcp/config` to retrieve MCP server status and configuration. The UI polls these endpoints periodically; dedicated log streaming is not required.
...
- **ProgressService**: Progress tracking via HTTP polling for long-running operations
