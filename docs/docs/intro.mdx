...
| **[Polling & Caching](./polling)** | HTTP polling with ETag and progress endpoints |
...
Archon now uses efficient HTTP polling with ETag caching:

- **Progress Endpoints**: `/api/progress/{operation_id}` with 304 ETag support
- **Smart Intervals**: Pauses when tab is hidden, resumes when active
- **Low Overhead**: 304 Not Modified dramatically reduces bandwidth
- **Consistent UX**: Predictable updates without WebSockets
